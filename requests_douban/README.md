...
```powershell
python -m requests_douban.cli --mode top250 --max-pages 1 --use-selenium --show-browser --driver-path "D:\tools\chromedriver.exe"
```
...
```powershell
python -m requests_douban.cli --mode top250 --max-pages 1 --no-details
```
...
```powershell
python -m requests_douban.cli --cookie "bid=xxxx; dbcl2=xxxx"
```
...
成员A当前代码主要位于 `requests_douban/`，可在报告中统计为独立模块。成员B可以读取 CSV/JSON 后写入 MySQL，或复用 `parser.py` 的字段结构改造成 Scrapy Item。成员C可以直接使用输出文件做数据分析、情感分析和图表生成。
